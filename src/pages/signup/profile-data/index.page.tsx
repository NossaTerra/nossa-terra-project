...
export default function ProfileDataScreen() {
  ...
  const onBuyerSuccessSubmit: BuyerFormProps["onSuccess"] = useCallback(
    ...
    [accountData, email, registerBuyer, router],
  );

  const registerSeller = api.auth.registerSeller.useMutation();
  const onSellerSuccessSubmit: SellerFormProps["onSuccess"] = useCallback(
    ...
    [accountData, email, registerSeller, router],
  );
    ...
    <div className="flex min-h-svh flex-grow flex-col">
      ...
      {role === "buyer" ? (
        <BuyerForm
          ...
          isLoading={registerBuyer.isLoading}
          formProps={{
            defaultValues: {
              avatarImage: picture,
            },
