...
function LoginGreetingContent({ className }: ClassNameProps) {
  ...
  const { checkUserKeys } = api.useUtils().auth;
  const onSubmit: SubmitHandler<FormData> = useCallback(
    async ({ email }) => {
      try {
        setIsLoading(true);
        const userKeys = await checkUserKeys.fetch({ email });
