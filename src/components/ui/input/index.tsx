  ...
  ({ suffix, className, maskPreset, onChange, ...props }, ref) => {
    ...
    return (
      <div className="flex items-center gap-3" ref={parentDivRef}>
        <input
          className={cn(
            "flex h-10 w-full rounded-md border border-slate-400 bg-backgroundPrimary px-3 py-2 text-sm text-black ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:opacity-50 dark:border-slate-800 dark:bg-slate-950 dark:ring-offset-slate-950 dark:placeholder:text-slate-400 dark:focus-visible:ring-slate-300",
            className,
          )}
          onInput={onInput}
          ref={ref}
