...
7. Run ``pnpm db:gen`` on the terminal to generate new schema
8. Run ``pnpm db:push``to push the new schema to the newly created branch
...
11. after changes are deployed you can put back the original value of the .env for APP_URL (the one that actually points to the main branch)
12. Run ``pnpm db:pull`` to get the data from planet scale remote main branch
13. Run ``pnpm db:push`` to make sure that the process was sucessful
